...
//	       fldOffset = fromTuple.copyFldOffset(); 
	  ...
	  public float getFloFld(int fldNo)
	    ...
	    float val;
	    if ((fldNo > 0) && (fldNo <= fldCnt)) {
	      val = Convert.getFloatValue(fldOffset[fldNo - 1], data);
	      return val;
	    } else
	      throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
	  }
	  ...
	  public String getStrFld(int fldNo)
	    ...
	    String val;
	    if ((fldNo > 0) && (fldNo <= fldCnt)) {
	      val = Convert.getStringValue(fldOffset[fldNo - 1], data,
	  ...
	  public Tuple setFloFld(int fldNo, float val)
	      throws IOException, FieldNumberOutOfBoundException {
	    if ((fldNo > 0) && (fldNo <= fldCnt)) {
	      Convert.setFloatValue(val, fldOffset[fldNo - 1], data);
	      return this;
	    } else
	      throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
	  ...
	  public Tuple setStrFld(int fldNo, String val)
	      throws IOException, FieldNumberOutOfBoundException {
	    if ((fldNo > 0) && (fldNo <= fldCnt)) {
	      Convert.setStringValue(val, fldOffset[fldNo - 1], data);
	      return this;
	    } else
	      throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
	  }
    ...
    switch (types[numFlds - 1].getAttrType()) {
  ...
  public void print(AttrType type[])
    ...
    for (i = 0; i < fldCnt - 1; i++) {
      switch (type[i].getAttrType()) {
        ...
        case AttrType.attrReal:
          fval = Convert.getFloatValue(fldOffset[i], data);
          System.out.print(fval);
          break;

        case AttrType.attrString:
          sval = Convert.getStringValue(fldOffset[i], data, fldOffset[i + 1] - fldOffset[i]);
    ...
    switch (type[fldCnt - 1].getAttrType()) {
