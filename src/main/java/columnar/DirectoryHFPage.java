 ...
 */
// TO-DO change prev and type to index and column no
public class DirectoryHFPage extends Page implements ConstSlot, GlobalConst {

    public static final int SIZE_OF_SLOT = 4;
    public static final int DPFIXED = 4 * 2 + 4 * 3 + 8;
    ...
    public static final int RECCNT = 20;
    ...
    public DirectoryHFPage(Page page) {
        data = page.getPage();
    }
    ...
    public void init(PageId pageNo, Page apage) throws IOException {
        ...
        usedPtr = (short) MINIBASE_PAGESIZE; // offset in data array (grow backwards)
     ...
     * @throws IOException I/O errors in C++ Status insertRecord(char *recPtr, int recLen,
     *                     RID& rid)
     */
    ...
    public byte[] getDataAtSlot(RID rid) throws IOException, InvalidSlotNumberException {
        short recLen;
        short offset;
        ...
        recLen = getSlotLength(slotNo);
        slotCnt = Convert.getShortValue(SLOT_CNT, data);
        byte[] ans = new byte[recLen];

        if ((slotNo >= 0) && (slotNo < slotCnt) && (recLen > 0) && (pageId.pid == curPage.pid)) {

            offset = getSlotOffset(slotNo);
            System.arraycopy(data, offset, ans, 0, recLen);
